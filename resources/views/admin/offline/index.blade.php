    ...
    <div class="modal fade" id="exLargeModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                ...
                <form method="POST" action="{{ route('offline.store') }}" enctype="multipart/form-data">
                    @csrf
    ...
    <div class="card">
        <div class="table-responsive text-nowrap">
            <table class="table">
                        ...
                        <td>
                            <div class="d-flex">
                                @if(auth()->user()->can('edit'))
                                    <a href="{{ route('offline.edit', $offline->id) }}"
                                ...
                                @if(auth()->user()->can('delete'))
                                    <button type="button" class="btn btn-icon btn-danger me-2" data-bs-toggle="modal"
                                            data-bs-target="#modalToggle{{$offline->id}}">
                                        <i class="bx bx-trash-alt"></i></button>

                                    <div class="modal fade" id="modalToggle{{$offline->id}}"
                                         aria-labelledby="modalToggleLabel{{$offline->id}}" tabindex="-1"
                                         style="display: none" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="modalToggleLabel{{$offline->id}}">
                                                        Buni qaytara olmaysiz!</h5>
                                                    <button type="button" class="btn-close"
                                                            data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">{{ $offline->title }}</div>
                                                <div class="modal-footer">
                                                    <form action="{{ route('offline.destroy', $offline->id) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                ...
                <!-- Модальное окно -->
                <div class="modal fade" id="imageModal{{$offline->id}}" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <img src="{{ asset('storage/'.$offline->image) }}" class="img-fluid"
                                     alt="Modal Image">
                            </div>
                        </div>
                    </div>
