                        ...
                        <tr>
                            ...
                            <td>
                                <ul class="list-unstyled users-list m-0 avatar-group d-flex align-items-center">
                                    <li
                                        data-bs-toggle="modal"
                                        data-bs-target="#imageModal{{$offline->id}}"
                                        data-bs-placement="top"
                                        class="avatar avatar-md pull-up"
                                    >
                                        <img src="{{ asset('storage/'.$offline->image) }}" class="rounded-circle"/>
                                    </li>
                                </ul>
                            </td>
                            <td>{!! $offline->desc !!}</td>
                            <td>
                                <div class="d-flex">
                                    ...
                                    @if(auth()->user()->can('delete'))
                                        <form
                                            action="{{ route('offline.destroy', $offline->id) }}"
                                            method="POST" id="form-delete">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
                                                    class="btn btn-icon btn-danger"
                                                    onclick="delete_button({{$offline->id}})">
